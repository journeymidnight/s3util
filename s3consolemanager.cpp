...
void S3ConsoleManager::Execute()
{
    ...
    case hash_compile_time("ls"):
        if (m_cli->para1 != "") {
            tmp = m_cli->para1;
            tmp.remove(0, 5);
            connect(this, &S3ConsoleManager::ContinueList, this, [ = ](QString bucketName, QString marker,
            QString prefix) {
                this->ListObjects(bucketName, marker, prefix);
            });
            if (tmp.contains('/')) {
                tmplist = tmp.split("/");
    ...
    case hash_compile_time("get"):
        ...
        if (tmp.endsWith('/')) {
            ...
            connect(this, &S3ConsoleManager::ContinuePrepareGet, this, [ = ](QString bucketName, QString marker,
            QString prefix, QString dstDir) {
                this->PrePareGetObjects(bucketName, marker, prefix, dstDir);
            });
...
void S3ConsoleManager::PrePareGetObjects(const QString &bucketName, const QString &marker,
                                         const QString &prefix, QString &dstDir)
{
    s3->Connect();
    ...
    connect(action, &ListObjectAction::ListObjectInfo, this, [ = ](s3object object,
    QString bucketName) {
        qDebug() << "enter insert to list\n";
        ObjectInfo info;
        info.bucketName = bucketName;
...
void S3ConsoleManager::ListObjects(const QString &bucketName, const QString &marker,
                                   const QString &prefix)
{
    ...
    connect(action, &ListObjectAction::ListPrefixInfo, this, [ = ](s3prefix prefix,
    QString bucketName) {
...
void S3ConsoleManager::PutObject(const QString &srcPath, const QString &bucketName,
                                 const QString &objectName)
{
    ...
    connect(handler, &ObjectHandlerInterface::updateProgress, this, [](uint64_t transfered,
    uint64_t total) {
...
void S3ConsoleManager::GetObject(const QString &bucketName, const QString &objectName,
                                 const QString &dstPath)
{
    ...
    this->h = pHandler;
    pHandler->start();
    connect(pHandler, &ObjectHandlerInterface::updateProgress, this, [](uint64_t transfered,
    uint64_t total) {
        // qDebug() << transfered << "/"<< total;
...
void S3ConsoleManager::GetObjects()
{
    ...
    s3->Connect();
    const ObjectInfo &item = m_objectList.constFirst();
