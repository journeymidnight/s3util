...
QS3Client::QS3Client(QObject *parent, QString endpoint, QString scheme, QString accessKey,
                     QString secretKey) : QObject(parent),
...
ListObjectAction *QS3Client::ListObjects(const QString &qbucketName, const QString &qmarker,
                                         const QString &qprefix, const QString &delimiter)
{
    //ListBucket
    Aws::String bucketName = QString2AwsString(qbucketName);
...
//        objects_request.WithMarker(marker).WithPrefix(prefix);
        ...
        if (list_objects_outcome.IsSuccess()) {
            const Aws::Vector<Aws::S3::Model::CommonPrefix> &common_prefixs =
            ...
            emit action->ListObjectFinished(true, list_objects_outcome.GetError(),
                                            list_objects_outcome.GetResult().GetIsTruncated(),
                                            AwsString2QString(list_objects_outcome.GetResult().GetNextMarker()));
        } else {
            ...
            emit action->ListObjectFinished(false, list_objects_outcome.GetError(),
                                            list_objects_outcome.GetResult().GetIsTruncated(),
                                            AwsString2QString(list_objects_outcome.GetResult().GetNextMarker()));
        }
    });
...
UploadObjectHandler *QS3Client::UploadFile(const QString &qfileName, const QString &qbucketName,
                                           const QString &qkeyName, const QString &qcontentType)
{
    auto clientHandler = new UploadObjectHandler(this, m_s3Client, qbucketName, qkeyName, qfileName,
                                                 qcontentType);
    return clientHandler;
}


DownloadObjectHandler *QS3Client::DownloadFile(const QString &bucketName, const QString &keyName,
                                               const QString &writeToFile)
{
